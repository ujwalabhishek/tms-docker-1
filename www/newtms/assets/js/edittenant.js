/* 
 * This js included in edit tenant page.
 */
$(document).ready(function() {
    ...
    function isunique_email(email) {
        ...
        $.ajax({
            ...
            type: 'post',
            data: {email: email, tenant_id: $('input[type=hidden][name=tenant_id]').val()},
            async: false,
            success: function(res) {
                output = false;
                if (res == 0) {
    ...
    function isunique_name(name) {
        ...
        $.ajax({
            ...
            type: 'post',
            data: {name: name, tenant_id: $('input[type=hidden][id=tenant_id][name=tenant_id]').val()},
            async: false,
            success: function(res) {
                output = false;
                if (res == 0) {
    ...
    function validate(retval) {
        ...
        if (logo.length == 0 && $('#user_image_preview img').attr('src') == undefined) {
    ...
    var search_tenant_name = $('#search_tenant_name');
    var tenant_id = $('#search_tenant_id');
    search_tenant_name.autocomplete({
        source: function(request, response) {
            search_tenant_name.val(search_tenant_name.val().toUpperCase())
            tenant_id.val('');
            $.ajax({
    ...
    $('#search_form').submit(function() {
        var tenant_name_err = $('#search_tenant_name_err');
        if (search_tenant_name.val().trim().length == 0) {
