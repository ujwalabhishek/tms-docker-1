...
//        $('#search_enrolment').text('Please Wait..');
        ...
        else if ($val == 'remove_invoice' || $val == 'add_invoice') 
        {
            ...
            /* skm code start when click on search button */
            $invoice_values = $.trim($('#company_invoice_id').val());
            $values = $invoice_values.split("#");
            $invoice_id = $values[0];
            $payid = $values[1];
            $course = $values[2];
            $class = $values[3]; 
            ...
            $('#company_name_1').text($company_name); // it shows company name on view page 
            $('#couse_class_name').text($course_name +' - '+$class_name); // it shows course and class name on view page
            $('#course_id').val($course); // it pass course id as a hidden value
            $('#class_id').val($class); // it pass class id as a hidden value
     ...
     // skm code start for drop down of invoice related to company
    $('#companies').change(function()
    {    
        $('#search_enrolment').hide(); // search button
        $company = $('#companies').val();
        $('#company_invoice_id').html('<option value="">Select</option>');
        $('#company_invoice_id').attr('disabled', 'disabled');
        if ($company.length > 0) { 
             $url = $siteurl + "class_trainee/get_company_all_invoice_for_remove";
            $.ajax({
                url: $url,
                type: "post",
                dataType: "json",
    ...
    $("#taxcode").autocomplete({
        ...
        select: function(event, ui) {
            var id = ui.item.key;
            $('#taxcode_id').val(id);
            $('#taxcode').val(ui.item.value);
            $('#course').removeAttr('disabled');
            
            check_nric_restrictions(ui.item.nric);  ///added by shubhranshu
             //added by shubhranshu for client requirement on 22/03/2019 //added by shubhranshu for client requirement on 22/03/2019
            ...
            success: function(data) {
                if (data != '') 
                {
                    ...
                    else
                    {
                        $.each(data, function(i, item) 
                        {
                             $('.control_6').append('<option value="' + item.user_id+'" data-id="'+item.tax_code+'">' + item.first_name + '( NRIC/FIN No. ' + item.tax_code + ' )</option>');
                        });
                        ////added by shubhranshu for nric check on 25/03/2019
                        //alert(data['blocklist'][0]);
//                        if(data['blocklist'] != ''){
//                            $.each(data['blocklist'], function (i, dat) {
//                                $j=1;
//                                $('#nric_list').html("");
//                               $('#nric_list').append("<b>"+$j+"."+dat+"</b>");
//                               $j++;
//                            });
//                            if($privilage == '0'){///added by shubhranshu
//                                if($role_id == 'ADMN'){///added by shubhranshu
//                                   $('#ex1111').modal();  ///added by shubhranshu/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  $('.add_inv').click(function(){
    ...
    $("#trainee").autocomplete({
        ...
        select: function(event, ui) {
            ...
            check_nric_restrictions(ui.item.nric);  ///added by shubhranshu
             //added by shubhranshu for client requirement on 22/03/2019
        },
        minLength: 4
    });
...
function check_nric_restrictions($nric){
    var $nric = $nric ? $nric : $("#taxcode").val(); 
    //alert($nric);
    if($nric != ''){
    $.ajax({
            ...
            dataType: "json",
            async:false,
            data: {tax_code: $nric,operation:'INDIVIDUAL_ENROL'},
            success: function(res) {
                if (res == 1) {
                    ///added by shubhranshu
                    if($privilage == '0'){
...
function form_validate($retval) {
   ...
   if ($val == "change" || $val == "remvind") 
    ...
    if ($account_type == 'individual') {
        ...
        $company_trainee = '';
        $trainies = [];///added by shubhranshu
        $s = 0;///added by shubhranshu
        $('input[name="control_6[]"]:checked').each(function() {
            $company_trainee += $(this).val();
            $trainies[$s] = $(this).val();///added by shubhranshu
            $s++;///added by shubhranshu
        })
        //////////////////////////////////added by shubhranshu for check of blocked nric/////////////////////////////////////////////////////
        if($company_trainee != ''){
            $.ajax({
                url: $siteurl + "manage_block_nric/get_blocked_nric_exist",
                type: "post",
                data: {nrics: $trainies,operation:'COMPANY_ENROL'},
                async: false,
                success: function(res) {
                    if(res == 1) {        
                        $('#restriction_flag').val('1');     
                    }else{
                       $('#restriction_flag').val('0');     
                    }
                }
            });
        }///////////////////////////////////////////////////////////////////////////////////////
...
function remove_button_disable(){
            ...
            var self = $('#remove_enrolment_change_form'),
            button = self.find('input[type="submit"],button');
            button.attr('disabled','disabled').html('Please Wait..');
            return true;
///////added by shubhranshu to prevent multiple clicks////////////////
...
function add_remove_invoice_validate() {
    ...
    if ($button_text == 'Remove Enrollment' || $button_text == 'Add Enrollment' || $button_text == 'Move Enrollment') {
        if ($(".trainee_check_box:checked").length > 0) {
            $('.check_box_alert').hide();
            ///////added by shubhranshu to prevent multiple clicks////////////////
            var self = $('#confirm_enrolment_change_form'),
            button = self.find('input[type="submit"],button');
            button.attr('disabled','disabled').html('Please Wait..');
            ///////added by shubhranshu to prevent multiple clicks////////////////
            return true;
