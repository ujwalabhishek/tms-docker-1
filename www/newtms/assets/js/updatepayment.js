...
//            $('#taxcode_id').val('');
    ...
    $('.search_button').click(function() {
        ...
        if ($account_type == 'company') {
            ...
            return false;
        } else {//added by shubhranshu for invoice search
            if ($search_select == 1) {
//                $invoice = $('#invoice').val();
...
//                }
                ...
                if ($invoice_no.length == 0) {
                    ...
                    disp_err('#invoice', '[Select Invoice No from auto-help]');
                } else {
                    trigger_ajax($invoice_id, $taxcode_id, $trainee_id);
                } //added by shubhranshu for invoice search
        ...
        if ($payid.length > 0) {
            ...
            $.ajax({
                ...
                success: function(res) {
                   ...
                   if(res.tenant.tenant_id='T17'){alert(res.tenant.tenant_id);
                       $('#payment_type').html('<option value="" selected="selected">Select</option><option value="CASH">Cash</option><option value="CHQ">Cheque</option><option value="SFC_SELF">SFC_SELF</option><option value="SFC_ATO">SFC_ATO</option>');
