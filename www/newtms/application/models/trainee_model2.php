...
//    }
   ...
   public function trainer_feedback($user_id, $course_id, $class_id) 
    ...
    {
        $this->db->select('cc.class_name,c.crse_manager,cc.classroom_trainer,cc.class_start_datetime,cc.class_end_datetime,c.crse_name,c.competency_code,'
                . 'c.crse_duration, tup.first_name, tup.last_name,tu.tax_code,tm.*');
        $this->db->from('class_enrol ce');
...
//        echo $this->db->last_query();
   ...
   /**
    ...
    {        
     ...
     public function get_course_manager_details($course_manager_id) 
    {        
        $tenantId = $this->session->userdata('userDetails')->tenant_id;
        $tids = explode(',', $course_manager_id);
        if (!empty($tids)) 
        {
            $course_manager_name = '';
            foreach ($tids as $tid) 
            {    
                ...
                $this->db->join('internal_user_role iur','iur.user_id = tu.user_id  and iur.tenant_id = tu.tenant_id');
                $this->db->where('tup.tenant_id',$tenantId);
                $this->db->where('iur.role_id','CRSEMGR');
                $this->db->where('iur.user_id',$tid);
                $query = $this->db->get();
                $data = $query->row(0);
                $course_manager = $data->first_name . ' ' . $data->last_name;

                $course_manager_name .="$course_manager,";
            }
            return rtrim($course_manager_name, ',');
        }
    }
