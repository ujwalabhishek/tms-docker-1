...
class Class_Model extends CI_Model {
    ...
    public function get_def_assessment_new_tpg($tenant_id, $class_id, $assmnt_type = '') {
        if ($assmnt_type == 'DEFAULT') {
            $result = $this->db->select('*')->from('class_assmnt_schld')->where('tenant_id', $tenant_id)
...
//       echo $this->db->last_query();
     ...
     * @return boolean
     */
    public function copy_classes_tpg($tenant_id, $course_name, $user_id, $data1, $tpg_course_run_id) {
...
//echo $this->db->last_query();exit;
    ...
    public function updateAssessmentData($score, $assessment_date, $grade, $user_id, $class_id, $course_id, $tenant_id, $assessment_ref_no) {
        $this->db->trans_start();
        $data = array(
        ...
        $this->db->where('class_id', $class_id);
        $this->db->where('user_id', $user_id);
        $this->db->where('assessment_reference_No', $assessment_ref_no);
        $this->db->update('class_enrol', $data);
        $this->db->trans_complete();
        if ($this->db->trans_status() === FALSE) {
            return FALSE;
