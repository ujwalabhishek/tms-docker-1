...
if ($value_of_schedule_class <= 0) {
    ...
    function check_for_assessment_checked($session_data, $date) {
        $format_date = $date->format('Y-m-d');
        if (isset($session_data[$format_date])) {
            $session_by_date_data = $session_data[$format_date];
            return $session_by_date_data['assmnt_attdn'] == '1';
        }
        return false;
    }
        ...
        if (!empty($tabledata) || !empty($attendance_status)) {
            ...
            <div class="row_dim1">            
                ...
                if (strtotime($class_start_date) <= strtotime(date('Y-m-d')) || $is_report_page) {
                                            ...
                                            for ($day = 0; $day < $days_count; $day++) {
                                                ...
                                                <td align="center">
                                                    ...
                                                    if (check_class_date_range($start_class, $end_class, $cur_date)) {
                                                        $checkbox_value = check_for_assessment_checked($data, $cur_date);
                                                        echo form_checkbox("mark_assessment[$key][$ses_date][assmnt_attdn]", '1', $checkbox_value);
                                                    }
                                                    ?>
                                                </td>
                                                <?php
                    ...
                    ?>
                    <div class="scroll"> 
                        <label class="red">This class has not yet started.</label>
                    </div>
                    <?php
                }
                ?>
